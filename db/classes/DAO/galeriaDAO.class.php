                ...
                for ($a = 0; $a < $qtdcol; $a++) {
                    ...
                    if ($celconstruida <= $cel) {
                        while ($dados = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            echo '<a href="#" style="text-decoration: none"><div class="card-header text-dark text-center" data-toggle="modal" data-target="#1">' . $gal_titulo . '</div>';
                            ...
                            echo '<div class="modal fade" id="1" tabindex="-1" role="dialog" aria-labelledby="2" aria-hidden="true"><div class="modal-dialog modal-dialog-centered" role="document"><div class="modal-content"><div class="modal-header">';
                            echo '<a href="index.php?&pg=foto&id=' . $dados['gal_cod'] . '">';
                            echo '<h5 class="modal-title text-dark" id="' . $dados['gal_cod'] . '">' . $gal_titulo . '</h5><a/>';
                            echo '<button type="button" class="close text-dark" data-dismiss="modal" aria-label="Close"><a href="index.php?&pg=foto&id=' . $dados['gal_cod'] . '"><span aria-hidden="true">&times;</span></button></div><div class="modal-body text-dark">';
                            if (file_exists('img/galeria/' . $gal_img) && !is_null($gal_img)) {
                                echo '<img src="img/galeria/'. $gal_img . '" alt="' . $gal_titulo . '" class="img-thumbnail" data-toggle="modal" data-target="#1">';
                            } else {
                                echo '<img src="img/galeria/semfoto.jpg" class="img-thumbnail" alt="Sem foto" data-toggle="modal" data-target="#1" >';
